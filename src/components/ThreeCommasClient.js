...
const signature = (secret, path, params) => 
                  HmacSHA256(params ? `${path}${params}` : path, secret).toString(Hex)
...
export default class ThreeCommasClient {
  ...
  async makeRequest(method, path, params) {
    ...
    for (let prop in params) queryParams.append(prop, params[prop])
  ...
  async getAccount(account_id) {
    return await this.makeRequest('GET', `/public/api/ver1/accounts/${account_id}`, null)
  }
...
export function useThreeCommasClient(mode = 'real') {
  ...
  let proxyPath = '/api/3commas'//'/.netlify/functions/cors-proxy/https://api.3commas.io'
